                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="form" action="{{ route('minute.update', compact('minute')) }}" method="post" enctype="multipart/form-data">
                            ...
                            <hr>
                            <div class="form-group">
                                <label for="signature_minute">
                                    <strong>{{ __('Unggah Notulen Yang Ditandatangani') }}</strong>
                                </label>
                                <div class="form-group card card-body">
                                    <label for="signature_minute"> Pemimpin Rapat </label>
                                    ...
                                    @if($minute->signature_minute)
                                        <p class="mb-0">
                                            Di unggah: <a target="_blank" href="{{ route('minute.signature', compact('minute')) }}">{{ __('Telah ditanda tangani oleh Pemimpin Rapat') }}</a>
                                        </p>
                                    @endif
                                </div>
                                <label for="signature_minute_notulis">
                                    <strong>{{ __('Unggah Notulen Yang Ditandatangani') }}</strong>
                                </label>
                                <div class="form-group card card-body">
                                    <label for="signature_minute_notulis"> Notulis Rapat </label>
                                    <input id="signature_minute_notulis" type="file" name="signature_minute_notulis" class="form-control-file" accept=".doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/jpeg,image/png,.pdf">
                                    @error('signature_minute_notulis')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror <hr>
                                    @if($minute->signature_minute_notulis)
                                        <p class="mb-0">
                                            Di unggah: <a target="_blank" href="{{ route('minute.signature-notulis', compact('minute')) }}">{{ __('Telah ditanda tangani oleh Notulis Rapat') }}</a>
                                        </p>
                                    @endif
                                </div>
                            </div>
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('minute.pdf', compact('minute')) }}" target="_blank" class="btn btn-primary mr-2">
                                    {{ __('PDF') }}
                                </a>
                                <a href="#" data-toggle="modal" data-target="#delete" class="btn btn-danger mr-2">
                                    {{ __('Hapus') }}
                                </a>
