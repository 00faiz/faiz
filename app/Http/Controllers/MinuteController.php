...
class MinuteController extends Controller
{
    ...
    public function update(UpdateMinute $request, Minute $minute)
    {
        ...
        if ($data->has('signature_minute_notulis')) {
            $signatureMinuteNotulis = $data->get('signature_minute_notulis');
            
            if ($signatureMinuteNotulis instanceof UploadedFile) {
                $oldSignatureNotulis = $minute->signature_minute_notulis;
                $originalNameNotulis = $signatureMinuteNotulis->getClientOriginalName();
                $fileNameNotulis = hash('sha256', Hash::make($originalNameNotulis)) . '.' . $signatureMinuteNotulis->getClientOriginalExtension();
                if ($signatureMinuteNotulis->move(storage_path('signature'), $fileNameNotulis)) {
                    $minute->update([
                        'signature_minute_notulis' => $fileNameNotulis,
                    ]);
                    
                    if ($oldSignatureNotulis) {
                        File::delete(storage_path('signature/' . $oldSignatureNotulis));
                    }
                }
            }
        }

        if ($minute->update($data->except(['presents', 'attachments', 'delete_attachments', 'signature_minute', 'signature_minute_notulis'])->toArray())) {
            return redirect()->route('minute.edit', compact('minute'))->with('notice', [
                'type' => 'success',
                'dismissible' => true,
    ...
    public function signature_notulis(Minute $minute)
    {
        $this->authorize('view', $minute);
        ...
        $filePath = storage_path('signature/' . $minute->signature_minute_notulis);
